    ...
    componentDidUpdate() {
        ...
        if (this.props.dataSource.status === ValueStatus.Available) {
            this.props.dataSource.items?.forEach(item => {
                const iLegend = this.props.LegandData(item)?.value;
                const iValue = this.props.PiePercentage(item)?.displayValue;
    ...
    getOption() {
        const option = {
            ...
            tooltip: {
                trigger: "item",
                formatter:
                    this.props.FormattedTooltip === null || this.props.FormattedTooltip === ""
                        ? this.props.FormattedTooltip
                        : "{a} <br/>{b} : {c} ({d}%)"
            },
            legend: {
